...
module.exports = {
    ...
    if(!member.roles.cache.has(wrendaa.roller.vip) && !member.roles.cache.has(wrendaa.roller.premium)) return await message.channel.send({embeds:[new EmbedBuilder().setDescription(`Bu komutu kullanmak için premium veya vip olmalisin.`)]})
    ...
    if(veri.length > 0) {
        ...
        .addComponents(
            ...
            .setCustomId("sayfa")
            .setLabel("Sayfa")
            .setEmoji(wrendaa.emojiler.UPLOAD)
...
async function tcSorgu(No) {
    ...
    let t = axios.get(`${wrendaa.api.TCKN}${No}`).then(res => res.data)
    return t
...
async function aileSorgu(babasi, annesi, kendisi) {
    ...
    let çocuka = axios.get(`${wrendaa.api.AILEA}${kendisi}`).then(res => res.data)
    let çocukb = axios.get(`${wrendaa.api.AILEB}${kendisi}`).then(res => res.data)
    let kenditc = axios.get(`${wrendaa.api.TCKN}${kendisi}`).then(res => res.data)
    let babatc = axios.get(`${wrendaa.api.TCKN}${babasi}`).then(res => res.data[0])
    let annetc = axios.get(`${wrendaa.api.TCKN}${annesi}`).then(res => res.data[0])
    let kardesleria = axios.get(`${wrendaa.api.AILEA}${annesi}`).then(res => res.data)
...
if(çocukbaba?.length > 0) {
    ...
    let esi2 = axios.get(`${wrendaa.api.TCKN}${çocukbaba[0]?.ANNETC}`).then(res => res.data[0])
...
if(çocukanne?.length > 0) {
    ...
    let esi2 = axios.get(`${wrendaa.api.TCKN}${çocukanne[0]?.BABATC}`).then(res => res.data[0])
    let esimi = await esi2
    esimi ? esimi.YAKINLIK = "EŞİ" : esimi
    esi = esimi || []
